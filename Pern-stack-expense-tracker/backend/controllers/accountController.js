  ...
  try {
    ...
    if(accountExist){
      return res.status(409).json({
        status:"failed",
        message:"Account alrady creatd"
      })
    }

    const createAccountResult = await pool.query({
      text:`INSERT INTO tblaccount(user_id, account_name, account_number, account_balance) VALUES($1,$2,$3,$4) RETURNING * FROM tblaccount WHERE`,
...
const initialDepositQuery={
  text:`INSERT INTO tbltransaction(user_id, description, type, status,amount,source) VALUES($1,$1,$3,$4,$5,$6),RETURNING`,
...
export const addMoneyToAccount = async(req, res)=>{
  try {
    ...
    const result = await pool.query({
      text:`UPDATE tblaccount SET account_balance=(account_balance+$1),updateat=CURREN_TIMESTAMP WHERE id=$2 RETURNING*`,
      values:[newAmount,id],
    })
    const accountInformation = result.rows[0];
    const description=accountInformation.account_name+"(Desposit)";
    const transQuery={
    ...
    res.status(200).json({
      status:"success",
      // message:accountInformation.account_name+"Account updated successfully",
